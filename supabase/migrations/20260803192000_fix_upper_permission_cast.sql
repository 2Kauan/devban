-- Fix: cast NEW.permission to text before calling UPPER() in notify_member_permission_update
  ...
  IF TG_OP = 'UPDATE' THEN
    IF NEW.permission != OLD.permission THEN
      ...
      VALUES (
        ...
        v_sender_name || ' alterou sua permissão para ' || UPPER(NEW.permission::text) || ' no projeto ' || v_project_name,
        'system',
