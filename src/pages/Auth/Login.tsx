      ...
      >
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-foreground">
            ...
            {view === 'email-sent' && 'E-mail enviado!'}
          </h2>
          <p className="text-muted-foreground mt-2 text-sm">
            {view === 'login' && 'Insira suas credenciais para acessar sua conta'}
            {view === 'forgot-password' && 'Digite seu e-mail para receber um link de redefinição'}
            {view === 'email-sent' && 'Verifique sua caixa de entrada para redefinir sua senha.'}
          </p>
        </div>
        ...
        {view === 'login' && (
          <>
            <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                {isLoading ? <div className="h-5 w-5 animate-spin rounded-full border-b-2 border-background"></div> : 'Entrar'}
              </button>
            </form>
        ...
        {view === 'forgot-password' && (
          <form className="space-y-4" onSubmit={handleForgotPassword}>
            ...
            <button 
              ...
              className="w-full h-11 flex justify-center items-center bg-primary text-primary-foreground font-semibold rounded-lg hover:bg-primary/90 transition-all active:scale-[0.98] disabled:opacity-70"
            >
              {isLoading ? <div className="h-5 w-5 animate-spin rounded-full border-b-2 border-background"></div> : 'Enviar Link de Recuperação'}
            </button>

            <button 
              type="button"
              onClick={() => setView('login')}
              className="w-full h-11 flex justify-center items-center bg-transparent border border-border text-foreground font-semibold rounded-lg hover:bg-muted transition-all active:scale-[0.98]"
            >
              Voltar para o Login
            </button>
          </form>
        ...
        {view === 'email-sent' && (
          <div className="space-y-6 text-center">
            <motion.div 
              ...
              className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 flex items-center justify-center text-primary mx-auto shadow-sm"
            >
              <MailCheck size={32} />
            </motion.div>
            ...
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground leading-relaxed">
                Enviamos um link de recuperação para o endereço <strong className="text-foreground">{resetEmail}</strong>.
              </p>
